...
println("Enter the number of clusters to use for K-means: ")
num_clusters = readline()  # Wait for user input to continue
println("User entered ", num_clusters, " clusters.")
num_clusters = parse(Int64, num_clusters)  # Convert the input to an integer
start_time = time()
...
CairoMakie.xlims!(ax_Kbar, 0, .5)
CairoMakie.ylims!(ax_Kbar, log_data[!,"DEPTH"][1], log_data[!,"DEPTH"][end])
# display(f5)
